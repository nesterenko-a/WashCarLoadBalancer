...
import { createAlgorithm, Dispatcher, generateArrivals, mulberry32, runSimulation, type CarWash, type SimConfig, type SimSnapshot, type SimulationResult, type VehicleType } from '@loadbalancer/core';
import './App.css';
...
function run(seed: number, algorithm: Algorithm, lambda: number, horizon: number) {
  ...
  const arrivals = generateArrivals({ lambdaBasePerMin: lambda, horizonMin: horizon, gridSizeMeters: 6000, typeShares: shares }, mulberry32(seed));
...
export default function App() {
  ...
  return <div className="app">
    ...
    <section className="center"><div className="map"><Map snapshot={snapshot} visibility={visibility}/>{!active && <div className="empty">Выберите алгоритм и нажмите «Запуск»</div>}</div><input className="timeline" type="range" min="0" max={maxTime || 1} step=".1" value={time} disabled={!active} onChange={e => setTime(Number(e.target.value))}/><div className="time-label"><span>{formatTime(time)}</span><span>{formatTime(maxTime)}</span></div>{active && <Table result={active} snapshot={snapshot}/>}</section>
...
function Map({ snapshot, visibility }: { snapshot?: SimSnapshot; visibility: Record<'roads' | 'posts' | 'queues' | 'values', boolean> }) { return <div className="factory"><div className="river"/>{visibility.roads && <><div className="road horizontal"/><div className="road diagonal"/><div className="roundabout"/></>}{WASHES.map((wash, index) => <Wash key={wash.id} wash={wash} state={snapshot?.washes.find(s => s.washId === wash.id)} color={colors[wash.id]} pos={index} visibility={visibility}/>)}</div>; }
