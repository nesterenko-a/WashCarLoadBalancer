...
function Map({ snapshot, visibility, route }: { snapshot?: SimSnapshot; visibility: Record<'roads' | 'posts' | 'queues' | 'values', boolean>; route?: { points: { coordinates: [number, number] }[] } }) { return <div className="factory"><div className="river"/>{visibility.roads && <><div className="road horizontal"/><div className="road diagonal"/><div className="roundabout"/></>}{route && <svg viewBox="0 0 6000 6000" preserveAspectRatio="none" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 2, pointerEvents: 'none' }}><polyline points={route.points.map(point => point.coordinates.join(',')).join(' ')} fill="none" stroke="#ffd166" strokeWidth="42" strokeLinecap="round" strokeLinejoin="round" opacity=".85"/></svg>}<div className="main-road">ОСНОВНАЯ ДОРОГА</div>{SOURCES.map((source, index) => <div className={'source source-' + index} key={source.id}><b>{source.kind === 'entrance' ? '↥' : '▰'} {source.name}</b><small>{source.kind === 'entrance' ? 'поток снаружи' : 'внутренний поток'}</small></div>)}{snapshot?.vehicles.filter(vehicle => vehicle.phase === 'transit').map(vehicle => <i key={vehicle.id} aria-label="Автомобиль в пути" style={{ position: 'absolute', zIndex: 4, width: 23, height: 12, borderRadius: 5, background: '#2879df', border: '2px solid #fff', boxShadow: '0 2px 5px #0008', left: vehicle.location[0] / 60 + '%', top: vehicle.location[1] / 60 + '%' }}/>) }{WASHES.map((wash, index) => <Wash key={wash.id} wash={wash} state={snapshot?.washes.find(s => s.washId === wash.id)} color={colors[wash.id]} pos={index} visibility={visibility}/>)}</div>; }
